...
namespace Vanslate.Helpers
{
    public static class Logger
    {
        ...
        private const string LogFile = "logs/bot.log";
        ...
        internal static void PrintHeader()
        {
            ...
            new Figlet().ToAscii("Vanslate").ConcreteValue.Split("\n", StringSplitOptions.RemoveEmptyEntries)
                .ForEach(ln => Info(LogSource.Bot, ln));
            Info(LogSource.Bot, Separator.Trim());
            Info(LogSource.Bot, $"Currently running Vanslate V{Version.FullVersion}.");
            _headerPrinted = true;
        }
        ...
        public static void Exception(Exception e) => Execute(LogSeverity.Error, LogSource.Bot, string.Empty, e);
        ...
        private static string NormalizeLogFilePath(DateTime date) => LogFile.Replace("bot", $"{date.Month}-{date.Day}-{date.Year}");
